...
- 手动补录任意时间点的一餐（自由时间选择，不限早/午/晚；也可以直接从菜单库点菜，不用手打菜名）
...
- **品类固定**：只有一套大类（盖浇饭/面食/粉面米线/麻辣烫·香锅/火锅/烧烤炸物/家常炒菜/轻食沙拉/快餐简餐/粥品/小吃/汤/甜品饮品/其他），导入、手填、编辑都从这里选
- **重新整理分类**：旧库里分类乱了，点「🪄 重新整理分类」让 AI 重判一遍 —— 先看改动清单，确认后才写库，可撤销
- **导出**：一键导出**文字版**（Markdown 清单，最适合贴给 AI 看，也能贴回本页「文字导入」重建菜单库）或 **JSON**（给程序/精确解析）；可复制也可下载成文件
- 手动添加、**按商家/品类两种方式浏览并可按品类筛选**、单个删除、恢复示例库；同商家同名自动去重
...
- **每日打卡**：喝水（按「我的杯子」录入，杯容量可设 100–1000ml、默认 250ml，也可直接按 ml 记）、步数（档位快选 / 滑块 / 直接输入）、睡眠、今日状态；进度条带「还差 X ml（约 X 杯）/ 还差 X 步（约 X km）」文案
- **体重与运动**：体重记录可补录最近 30 天内的任意一天（补录不改档案体重），趋势曲线看方向；运动记录记类型 / 时长 / 距离与里程碑
...
### 递增规则（语义化版本，**成块的变化就用成块的号**）

| 变更类型 | 命令 | 例子 |
|---|---|---|
| 不兼容改动（数据格式、交互大改） | `npm version major` | 1.0.0 → 2.0.0 |
| **成块的新功能**（新增一个完整模块/页面/能力） | `npm version minor` | 1.0.0 → 1.1.0 |
| **成块的界面 / 交互改版**（布局、导航、信息层级、可读性整体调整） | `npm version minor` | 1.5.5 → 1.6.0 |
| **其余一切**：修 bug、文案调整、加一个按钮、小功能增强 | `npm version patch` | 1.0.0 → 1.0.1 |
| 只改文档 / 注释 / 构建脚本 | 不升版本 | — |

> 判断依据是"用户能感知到的变化有多成块"，不是"改了多少行代码"。
> 界面改版尤其别看行数：改一个 CSS 变量和重排一次版式，在用户眼里都是"界面变了"。
