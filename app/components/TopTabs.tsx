...
/**
 * 顶部的两个平级入口：🍳 今天吃什么 ｜ 💪 健康小屋。
 *
 * 为什么要有这个组件：原先健康小屋只是首页标题右边三个小按钮之一，
 * 和「本周回顾」同样大小 —— 看起来像附属功能。改成两个同尺寸的大入口后，
 * 健康与"吃"在视觉上同等份量，且在两个页面里都常驻（在健康小屋也能一键回到今天吃）。
 *
 * 用 `active` 显式传入而不用 `usePathname()`：静态导出 + 子路径部署下，
 * pathname 是否带 basePath 容易踩坑，而这里本来就知道自己是哪一页。
