...
export const ACTIVITY_LEVELS: { label: ActivityLevel; desc: string; factor: number; stepsTarget: number }[] = [
...
/** Mifflin-St Jeor 公式，成人适用的基础代谢估算 */
...
/** 目标热量：TDEE 加目标偏移，并卡在安全线下方之上（学生党不鼓励激进节食） */
...
/** 把健康档案 + 今日打卡拼成一段注入 AI prompt 的上下文，没有档案时返回空串 */
export function buildHealthContext(
  ...
  if (today) {
    const parts = [`喝水 ${today.waterMl}ml（目标 ${t.waterTarget}ml）`, `步数 ${today.steps}（目标 ${t.stepsTarget}）`];
